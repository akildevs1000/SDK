    ...
    public class CommandAllocator
    {
        ...
        private static void CardTransaction(Door8800Transaction transaction)
        {
            ...
            if (transaction.CmdIndex == 0x01)
            {
                ...
                record.RecordNumber = (uint)cardtrn.SerialNumber;
                ...
                if (record.Photo == 0)
                {
                    SendWebSocketMessage(GetResult(Command.CommandStatus.Succeed, TransactionType.FaceTransaction, record), record.RecordNumber);
                    return;
                }
        ...
        private static IFcardCommandResult GetResult(Command.CommandStatus status, TransactionType type, object data = null)
        {
        ...
        private static void DefaultTransaction(Door8800Transaction transaction)
        {
            ...
            {
                RecordNumber = (uint)transaction.EventData.SerialNumber,
        ...
        {
            KeepAliveTransaction record = new KeepAliveTransaction
            {
                Sn = transaction.SN,
                KeepAliveTime = transaction.EventData.TransactionDate.ToDateTimeStr()
            };
        ...
        private static void BeginWatch(string sn, MyConnectorDetail myConnectorDetail)
        {
